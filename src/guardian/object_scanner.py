    ...
    try:
        with open(pack_path, 'rb') as f:
            ...
            try:
                ...
                raise GitObjectError(
                    f"Failed to decompress object data at offset {offset}: CRC error or corrupted data"
                )
    ...
    try:
        ...
        for i in range(header.object_count):
            try:
                ...
                raise GitObjectError(f"Invalid CRC at offset {offset + 12}: {e}")
...
def iter_objects(repo_path: Path) -> Iterator[GitObject]:
    ...
    for prefix_dir in objects_dir.iterdir():
        ...
        if len(prefix_dir.name) != 2 or not all(c in '0123456789abcdef' for c in prefix_dir.name):
            continue
